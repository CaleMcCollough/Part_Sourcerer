...
# @date         01/15/2015
# @brief        This file contains the Metascrapper class and command-line application
# @description  Metascrapper is a set of Electronic Design Automation tools written in Python.
#
# Supported File Formats:
# Metascrapper currently only supports the DipTrace ASCII file format(in progress. See www.diptrace.com), but in the future will 
...
#               format.The purpose of Metascrapper is to serve as an automation tool for processing EDA design files. Metascrapper is really
#
...
@click.command()
@click.option('-s', default='', help = 'This option runs the EDA Scrapper on the specified.')
@click.option('-app', default='', help = 'The name of the EDA Application.')
def cli(libraryFilename, edaSoftareType):
    """Welcome to the Electronics Design Automation Scrapper."""
...
class Scrapper():
    ...
    @staticmethod
    def printLine(char):
        (width, height) = Scrapper.getTerminalSize()
        line = ''
    ...
    def fixFilename(filename):
        valueURL = ''.join(c for c in filename if c in Scrapper.validChars)
        #print properURL
        return valueURL
    ...
    def saveToFile(data, filename):
        if not Scrapper.pathIsValid(filename):
            print 'Error: Filename not valid: ', filename
            return
    ...
    class UnitTest():
        @staticmethod
        def runUnitTest ():
            print "Running Scrapper.UnitTest...\n"
            
            #edaTools = Scrapper()
        
            print "Done running Scrapper.UnitTest.\n"

Scrapper.UnitTest.runUnitTest()